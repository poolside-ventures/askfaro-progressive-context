...
`askfaro-progressive-context` is dependency-free at its core; features that need
...
```bash
pip install askfaro-progressive-context            # core: format, runtime, eval
pip install 'askfaro-progressive-context[llm]'     # + OpenAI-compatible client (httpx)
pip install 'askfaro-progressive-context[schema]'  # + full JSON Schema validation
pip install 'askfaro-progressive-context[tokenize]'# + exact tiktoken counts
pip install 'askfaro-progressive-context[dev]'     # + pytest
```
...
|---|---|---|
| `OpenAICompatibleClient needs httpx. Install the 'llm' extra` | building against a real model without the client dependency | `pip install 'askfaro-progressive-context[llm]'` |
...
| `JSON Schema validation requires the 'schema' extra` | `pcx validate --schema` without jsonschema | `pip install 'askfaro-progressive-context[schema]'`, or drop `--schema` for the zero-dep structural check |
